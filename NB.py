...
def save_lyrics_label(lyrics_file, labels_file):
    ...
    for genre in genres:
        with open("./data/lyrics/test_{}.json".format(genre)) as f:
            lyr = extract_lyrics_from_file(f, num=None)
            label = [genre2int[genre]] * len(lyr)
            lyrics.extend(lyr)
            labels.extend(label)
            print("genre:{}, has {} lyrics, label length{}".format(genre, len(lyr), len(label)))
