...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=
        "Save gathered data into a json file. Read data from saved json file, make a classifier and then evaluate its performance.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        "--file", nargs='?', required=True, help="input/output file")
    parser.add_argument(
        "--authors", nargs='+', required=True, help="authors' names")
    parser.add_argument(
        "--emails", nargs='+', required=True, help="authors' email addresses")
    parser.add_argument(
        "--description", nargs='+', required=True, help="project description")
    parser.add_argument(
        "--readfile",
        nargs='?',
        default=None,
        help=
        "tab separated data file that will be encoded into json and written (clobbers --file). if not present, assume we're writing"
    )
